...
def parse_args():
    ...
    parser.add_argument('--img_dir', type=str,
                        default='checkpoints/CloSE_eva_SAM/imgs_upsampled/')
    parser.add_argument('--inp_pose_params_dir', type=str,
                        default='checkpoints/CloSE_eva_SAM/tokenhmr_output/')
    parser.add_argument('--garmentcode_dir', type=str,
                        default='runs/try_7b_lr1e_4_v3_garmentcontrol_4h100_v4_final_eva/vis_new/')
    parser.add_argument('--template_smpl_pkl', type=str,
                        default='/home/ids/liliu/projects/ChatGarment/assets/eval/aaa_mesh_registrarion/registered_params_garmentgenerator_SMPL.pkl')
    parser.add_argument('--saved_dir', type=str,
                        default='checkpoints/CloSE_eva_SAM/postprocess/')
    parser.add_argument('--garment_seg_dir', type=str,
                        default='checkpoints/CloSE_eva_SAM/mask/')
    args = parser.parse_args()

    new_args = edict(
        inp_img=search_path(args.img_dir, args.imgname, '.png'),
        inp_pose_params_path=search_path(
            args.inp_pose_params_dir, args.imgname, '_0.pkl'),
        template_smpl_pkl=args.template_smpl_pkl,
        person_seg_path=search_path(
            args.garment_seg_dir, args.imgname, '_person.png'),
    )

    garmentcode_dir = os.path.join(
        args.garmentcode_dir, f'valid_garment_{args.imgname}')
    ...
    for filename in all_seg_names:
        ...
        if segname in possible_garment_names:
            ...
            this_args['skirt_seg_path'] = os.path.join(
                args.garment_seg_dir, f'{args.imgname}_{segname}.png')
            saved_path = os.path.join(
                args.saved_dir, args.imgname, f'valid_garment_{garment_type}')
            ...
            print(
                f'Warning: garment type {segname} not in possible_garment_names')
            continue
...
def evaluate_wrapper_base(opt_params, ori_params_all, saved_dir, name='upd',
    ...
    json_path = os.path.join(
        saved_dir, f'valid_garment_{name}', f'valid_garment_{name}_specification.json')
    command = f'python run_garmentcode_sim.py --json_spec_file "{json_path}"'
    subprocess.run(command, shell=True, text=True)

    # LBS re-pose
    mesh_path = os.path.join(
        saved_dir, f'valid_garment_{name}', f'valid_garment_{name}', f'valid_garment_{name}_sim.obj')
...
def criterion(garment_seg, person_seg, pred_seg_init, pred_seg):
    assert garment_seg.shape == person_seg.shape, (
        garment_seg.shape, person_seg.shape)
    assert garment_seg.shape == pred_seg.shape, (
        garment_seg.shape, pred_seg.shape)
...
def FDM(args):
    ...
    garment_seg, person_seg, pant_seg = parse_sam_results(
        args.skirt_seg_path, args.person_seg_path, is_upper_garment=is_upper_garment)
    garment_seg = cv2.resize(garment_seg, img_size)
    person_seg = cv2.resize(person_seg, img_size)

    gt_seg_vis = img * 0.5 + \
        garment_seg.reshape(img_size[0], img_size[1], 1) * 0.5
    cv2.imwrite(os.path.join(args.saved_path, 'gt_seg_vis.png'), gt_seg_vis)

    garment_seg = torch.from_numpy(
        np.array(garment_seg)).squeeze(-1).float().to(DEVICE) / 255.0
    person_seg = torch.from_numpy(
        np.array(person_seg)).squeeze(-1).float().to(DEVICE) / 255.0
    if pant_seg is not None:
        pant_seg = cv2.resize(pant_seg, img_size)
        pant_seg = torch.from_numpy(
            np.array(pant_seg)).squeeze(-1).float().to(DEVICE) / 255.0
    ...
    inp_smpl_params = {}
    inp_smpl_params['betas'] = torch.from_numpy(
        inp_smpl_params_origin['pred_smpl_params']['betas']).float().reshape(1, 10).to(DEVICE)
    ...
    inp_smpl_params['poses'] = torch.from_numpy(
        inp_smpl_params['poses']).float().to(DEVICE)
    inp_smpl_params['transl'] = torch.from_numpy(
        inp_smpl_params_origin['pred_cam_t_full']).float().reshape(1, 3).to(DEVICE)
    inp_smpl_params['focal_length'] = inp_smpl_params_origin['scaled_focal_length'] / 4096 * IMG_LENGTH

    template_smpl_params = {}
    template_smpl_params['betas'] = torch.from_numpy(
        template_smpl_params_origin['pred_shape']).float().reshape(1, 10).to(DEVICE)
    template_smpl_params['poses'] = torch.from_numpy(
        template_smpl_params_origin['pred_pose']).float().reshape(1, 24, 3).to(DEVICE)
    template_smpl_params['transl'] = torch.from_numpy(
        template_smpl_params_origin['pred_transl']).float().reshape(1, 3).to(DEVICE)

    def evaluate_wrapper(opt_params, configRC, saved_dir, name): return evaluate_wrapper_base(
        opt_params, configRC, saved_dir, name, template_smpl_params, inp_smpl_params)
    ...
    optimizer = torch.optim.Adam(
        [item for item in opt_params.values()], lr=0.025, betas=(0.5, 0.9))
    lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        optimizer, max_iterations, eta_min=0.01)

    losses = []
    pred_seg_init = None
    best_loss = 1e10
    for i in range(max_iterations):
        pred_seg = evaluate_wrapper(
            opt_params, configRC, args.saved_path, 'base')
        if pred_seg_init is None:
            pred_seg_init = pred_seg.detach().clone()
            img_overlap = img * 0.5 + \
                pred_seg.detach().cpu().numpy().reshape(
                    img_size[0], img_size[1], 1) * 0.5 * 255
            cv2.imwrite(os.path.join(args.saved_path,
                        'img_overlap_init.png'), img_overlap)
        ...
        img_overlap = img * 0.5 + \
            pred_seg.detach().cpu().numpy().reshape(
                img_size[0], img_size[1], 1) * 0.5 * 255
        cv2.imwrite(os.path.join(args.saved_path,
                    f'img_overlap_{i}.png'), img_overlap)
        ...
        for name, param in opt_params.items():
            delta_param = 0.06 if name in [
                'pants.flare', 'shirt.width', 'sleeve.end_width'] else 0.02
            opt_params_tmp = copy.deepcopy(opt_params)
            opt_params_tmp[name] = param + delta_param
            pred_seg_pert = evaluate_wrapper(
                opt_params_tmp, configRC, args.saved_path, name)
            loss_pert = criterion(garment_seg, person_seg,
                                  pred_seg_init, pred_seg_pert)
        ...
        saved_dict = {k: f'{v.detach().cpu().item():.5f}' for k,
                      v in opt_params.items()}
